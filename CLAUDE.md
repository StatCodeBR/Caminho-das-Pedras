...
Este projeto usa OpenSpec. **Não implemente nada sem uma mudança aprovada em
`openspec/changes/`.**

O ciclo é: `/opsx:propose` → revisão humana da spec → `/opsx:apply` →
`openspec validate --strict` → `/opsx:archive`.

As instruções detalhadas do OpenSpec estão em `openspec/AGENTS.md`, que é a
fonte de verdade sobre formato de spec e comandos disponíveis.
...
4. **Dois projetos Python separados.** `pipeline/` carrega torch; `api/` não pode
   carregar. Nunca unifique as dependências.

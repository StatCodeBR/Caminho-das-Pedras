...
Aproveitar a complementaridade entre a busca léxica e a semântica. O BM25 acerta
sigla, nome de órgão e termo exato, e falha quando o vocabulário do usuário não
coincide com o do catálogo; o embedding faz o oposto. Usar só uma delas aceita de
graça a fraqueza correspondente.

A fusão é por posição, não por pontuação: as duas escalas são incomparáveis — o
BM25 não tem teto e depende do corpus, o cosseno vive entre menos um e um — e
combiná-las com pesos exigiria recalibrar a cada recatalogação.

Esta capacidade é o ponto único de entrada da recuperação: quem responde não
precisa saber de qual ranking cada conjunto veio, embora possa consultar essa
procedência.

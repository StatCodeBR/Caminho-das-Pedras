...
Entregar ao usuário uma resposta que ele possa conferir. É a camada que o
cidadão efetivamente toca e onde o projeto pode se destruir sozinho: um
assistente sobre dados públicos que inventa o nome de um conjunto, ou devolve
uma URL plausível e inexistente, é pior que nenhum assistente — desperdiça o
tempo de quem confiou nele e desmoraliza a própria ideia de reúso de dados
abertos.

A garantia não vem de instruir o modelo a não inventar, e sim de duas medidas
estruturais: ele só recebe as fichas recuperadas, e a saída é conferida antes de
chegar ao usuário. URL fora do contexto significa fabricação, e a resposta
inteira é descartada — nunca remendada.

Há ainda uma decisão de custo com efeito direto no produto: quando a recuperação
é conclusiva, a resposta sai por template, sem consumir token. O modelo entra
quando há ambiguidade — que, medido, é a maioria dos casos.
